...
src_prepare() {
	...
	if ! use system-aom; then
		keeplibs+=( third_party/libaom )
		keeplibs+=( third_party/libaom/source/libaom/third_party/fastfeat )
		keeplibs+=( third_party/libaom/source/libaom/third_party/SVT-AV1 )
	...
	if use system-vpx; then
		cp ${FILESDIR}/vpx_thread.h ${WORKDIR}/${P}/third_party/libvpx/source/libvpx/vpx_util/
		cp ${FILESDIR}/vpx_config.h ${WORKDIR}/${P}/third_party/libvpx/source/libvpx/
		cp ${FILESDIR}/vpx_config.h ${WORKDIR}/${P}/third_party/libvpx/source/libvpx/vpx_util/
	fi
	if use system-aom; then
		cp ${FILESDIR}/aom_thread.h ${WORKDIR}/${P}/third_party/libaom/source/libaom/aom_util/
	fi

	sed '/"-Zdep-info-omit-d-target",/d' -i ${WORKDIR}/${P}/build/config/compiler/BUILD.gn
	sed '/"-Zmacro-backtrace",/d' -i ${WORKDIR}/${P}/build/config/compiler/BUILD.gn
	sed '/"-Zremap-cwd-prefix=.",/d' -i ${WORKDIR}/${P}/build/config/compiler/BUILD.gn
	sed '/"-Zexternal-clangrt",/d' -i ${WORKDIR}/${P}/build/config/compiler/BUILD.gn
